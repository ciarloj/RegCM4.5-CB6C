...
!
  ...
  public :: drydep_aero , drydep_gas , aerodyresis , drydep_soa
...
!******************************************************************************
!
    subroutine drydep_soa (j,mbin,n1bin,n2bin,rhop,ivegcov,throw,    &
...
! output table to be passed out. Care dimension is ntr
      ...
      real(rk8) , intent(out) , dimension(ici1:ici2,kz,mbin) :: pdepv
      real(rk8) , intent(out) , dimension(ici1:ici2,mbin) :: ddepv
!
      real(rk8) :: amfp , amob , eb , eim , ein , frx1
      real(rk8) :: pre , prii , priiv , r1 , st
      real(rk8) , dimension(ici1:ici2,kz) :: amu
      ...
      integer(ik4) :: i , k , kcov , l , n , ib
#ifdef DEBUG
      character(len=dbgslen) :: subroutine_name = 'drydep_soa'
      integer(ik4) , save :: idindx = 0
      call time_begin(subroutine_name,idindx)
#endif
      ! here avesize is a RADIUS of the particle bin in m :
...
!======================================================================
      ...
      do ib = n1bin , n2bin
        ...
        ddepv(:,ib)   = 0.0D0
        do i = ici1 , ici2
          pdepv(i,:,ib) = pdepvsub(i,:,ib)
          ! agregate the dry deposition velocity, remember one cover per grid
          ! cell for now
          ! the dry deposition deposition velocity must accound also for the
          ! settling vrlocity at kz
          ! simple form now add the vs
          ddepv(i,ib) = 1.0D0/(ra(i,1)+rs(i,1,ib)) + pdepvsub(i,kz,ib)
        end do
      end do
      !
      ! Finally update the emission and settling tendencies for
      ...
      do ib = n1bin , n2bin
        ...
        do k = 2 , kz
          do i = ici1 , ici2
            wk(i,k) =  twt(k,1)*chib(j,i,k,ib)+ &
                       twt(k,2)*chib(j,i,k-1,ib)
          end do
        end do
        do i = ici1 , ici2
          do k = 2 , kz - 1
            ! do not apply to the first level
            !settend(i,k) = (wk(i,k+1)*pdepv(i,k+1,ib) - &
            !                wk(i,k)*pdepv(i,k,ib)) / cdzq(j,i,k)
            ! use exponential form for stability
            settend(i,k) =  wk(i,k+1) * (d_one - &
              dexp(-ddepv(i,ib)/cdzq(j,i,k)*dt ))/dt  - &
                            wk(i,k)   * (d_one - &
              dexp(-ddepv(i,ib)/cdzq(j,i,k)*dt ))/dt

            chiten(j,i,k,ib) = chiten(j,i,k,ib) - settend(i,k)
            if ( ichdiag == 1 ) then
              cseddpdiag(j,i,k,ib) = cseddpdiag(j,i,k,ib) - &
                                            settend(i,k) * cfdout
            end if
          end do
          !
          ...
          if ( ichdrdepo == 1 ) then
            ...
            ! settend(i,kz) =  (chib(j,i,kz,ib)  * ddepv(i,ib)-  &
            !                   wk(i,kz)*pdepv(i,kz,ib)) / cdzq(j,i,kz)
            settend(i,kz) =  chib(j,i,kz,ib) * &
              (d_one - dexp(-ddepv(i,ib)/cdzq(j,i,kz)*dt ))/dt -  &
                             wk(i,kz) * &
              (d_one - dexp(-pdepv(i,kz,ib)/cdzq(j,i,kz)*dt ))/dt
            chiten(j,i,kz,ib) = chiten(j,i,kz,ib) - settend(i,kz)
            ...
            cdrydepflx(j,i,ib) = cdrydepflx(j,i,ib) + &
              (chib(j,i,kz,ib)-settend(i,kz)*dt/d_two) / cpsb(j,i) *  &
               crhob3d(j,i,kz)* ddepv(i,ib)

            !diagnostic for settling and drydeposition removal
            if ( ichdiag == 1 ) then
              cseddpdiag(j,i,kz,ib) = cseddpdiag(j,i,kz,ib) - &
                                             settend(i,kz) * cfdout
            end if
            ! accumulated diagnostic for dry deposition flux
            ! average (in kg .m2.s-1)

            remdrd(j,i,ib) = remdrd(j,i,ib) + &
                                    cdrydepflx(j,i,ib) * cfdout

            ! no net flux is passed to BL schemes in this case
            chifxuw(j,i,ib) = d_zero
            drydepv(j,i,ib) = d_zero
            ...
            ! for the BL scheme !
            ! flux
            chifxuw(j,i,ib) = chifxuw(j,i,ib) - &
                chib(j,i,kz,ib)/ cpsb(j,i) * ddepv(i,ib)
          end if
          !
          ! dry dep velocity diagnostic in m.s-1  ( + drydep v. include
          ! also settling , accumulated between two outputs time step)
          ddv_out(j,i,ib) = ddv_out(j,i,ib) + &
                 ddepv(i,ib)
        end do
      end do
#ifdef DEBUG
      ...
      ! vdg in m.s-1
       ...
       if ( igaschem > 0 ) then
         if ( chemsimtype(1:4) == 'CB6C' ) then
           drydepvg = d_zero
           drydepvg(:,isdio) =  vdg(1,:,1)
           drydepvg(:,isulf) =  vdg(2,:,1)
           drydepvg(:,indox) =  vdg(3,:,1)
           drydepvg(:,iozn)  =  vdg(4,:,1)
           drydepvg(:,ihpox) =  vdg(5,:,1)
           drydepvg(:,intrc) =  vdg(6,:,1)
           drydepvg(:,ihono) =  vdg(7,:,1)
           drydepvg(:,ipna ) =  vdg(8,:,1)
           drydepvg(:,ipacn) =  vdg(10,:,1)
           drydepvg(:,iform) =  vdg(14,:,1)
           drydepvg(:,iaald) =  vdg(15,:,1)
           drydepvg(:,imegy) =  vdg(22,:,1)
           drydepvg(:,imeoh) =  vdg(23,:,1)
           drydepvg(:,ietoh) =  vdg(24,:,1)
         else
           drydepvg = d_zero
...
#ifdef CLM
           if ( ivegcov(i) == 0 ) then
             drydepvg(i,1:numtr)  =  cdep_vels(j,i,1:numtr)
           end if
#endif
           do n = 1 , numtr
