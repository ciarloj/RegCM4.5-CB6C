...
!
    subroutine tractend2(ktau,lyear,lmonth,lday,calday,declin)
      ...
      real(rk8) , dimension(ici1:ici2,kz,ntr) :: chibset
...
!            end if
      ...
      !
      ! prepare density and diameter values for SOAs
      !
      if ( isorgam == 1 ) then
        dian(:) = d_zero
        diaa(:) = d_zero
        ...
        do ic = 1 , 3*ngstr
          do j = jci1 , jci2
            do i = ici1 , ici2
              do k = 1 , kz
                dian(ic) = dian(ic) + dgn(j,i,k,trac%indcb6(ic))
                diaa(ic) = diaa(ic) + dga(j,i,k,trac%indcb6(ic))
                nmb(ic)  = nmb(ic)  + 1
                dnuc     = dnuc + dnsn(j,i,k,trac%indcb6(ic))
                dacc     = dacc + dnsa(j,i,k,trac%indcb6(ic))
                numb     = numb + 1
              end do
            end do
          end do
          dian(ic) = dian(ic)/nmb(ic)
          diaa(ic) = diaa(ic)/nmb(ic)
        end do
        dnuc = dnuc/numb
        dacc = dacc/numb
      end if
      ...
      if ( (iso2 > 0 .or. igaschem == 1) .and. ichdrdepo > 0 ) then
        if ( isorgam == 1) then
          ...
          do j = jci1 , jci2
            call drydep_soa(j,3*ngstr,ngstr+1,2*ngstr,dnuc,ivegcov(:,j),  &
                             ttb(:,:,j),rho(:,:,j),hsigma,psurf(:,j),     &
                             temp10(:,j),tsurf(:,j),srad(:,j),            &
                             rh10(:,j),wid10(:,j),zeff(:,j),              &
                             dian,pdepv(:,:,:,j),ddepa(:,:,j))
          end do
          do j = jci1 , jci2
            call drydep_soa(j,3*ngstr,2*ngstr+1,3*ngstr,dacc,ivegcov(:,j),&
                             ttb(:,:,j),rho(:,:,j),hsigma,psurf(:,j),     &
                             temp10(:,j),tsurf(:,j),srad(:,j),            &
                             rh10(:,j),wid10(:,j),zeff(:,j),              &
                             diaa,pdepv(:,:,:,j),ddepa(:,:,j))
          end do
        else
          do j = jci1 , jci2
            call drydep_gas(j,lmonth,lday,ivegcov(:,j),rh10(:,j), &
        ...
        if ( isorgam == 1) then
          do j = jci1 , jci2
            ...
            call sethet(j,hgt(:,:,j),hsurf(:,j),ttb(:,:,j),        &
                        ...
                        dt,rho(:,:,j),chibset,psurf(:,j))
          end do
          do j = jci1 , jci2
            call wetdepsoa(j,3*ngstr,ngstr+1,2*ngstr,dian,dnuc,ttb(:,:,j),     &
                         wl(:,:,j),fracloud(:,:,j),fracum(:,:,j),psurf(:,j),   &
                         hsigma,rho(:,:,j),prec(:,:,j),convprec(:,:,j),        & 
                         pdepv(:,:,:,j),trac%indcb6)
          end do
          do j = jci1 , jci2
            call wetdepsoa(j,3*ngstr,2*ngstr+1,3*ngstr,diaa,dacc,ttb(:,:,j),   &
                         wl(:,:,j),fracloud(:,:,j),fracum(:,:,j),psurf(:,j),   &
                         hsigma,rho(:,:,j),prec(:,:,j),convprec(:,:,j),        &
                         pdepv(:,:,:,j),trac%indcb6)
          end do
      ...
      if ( igaschem == 1 .and. ichsolver > 0 ) then
        if ( mod(ktau+1,kchsolv) == 0 ) then
          ...
          if ( chemsimtype(1:4) == 'CB6C' ) then
            ...
            if ( isorgam == 1 ) then
              do j = jci1, jci2
                call chemistry_srg(j,cerr)
                if ( cerr == 1 ) EXIT
              end do
            else
              do j = jci1 , jci2
                call chemistry_cb6(j,cerr)
                if ( cerr == 1 ) EXIT
              end do
            end if
            if ( myid == italk .and. mod(ktau+1,krep) == 0 ) then
              write(stdout,'(a,2g12.5)') ' $$$ Jvalue min/max NDOXsurf : ', &
                minval(jphoto(:,:,kz,jvNDOX )),  maxval(jphoto(:,:,kz,jvNDOX ))
            end if
          else
            do j = jci1 , jci2
              call chemistry(j)
