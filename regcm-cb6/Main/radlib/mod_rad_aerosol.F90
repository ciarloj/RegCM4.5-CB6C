...
!
  ...
  ! optical properties for sorgam cb6 species following standard scheme only
  real(rk8) , pointer , dimension(:,:) :: kasoa , wasoa , gasoa
  ...
  contains
    ...
    subroutine allocate_mod_rad_aerosol
      implicit none
      integer(ik4) :: nband , ns , itr , numtr
      ...
      if ( ichem == 1 ) then
        call getmem3d(fa,1,npoints,0,kz,1,numtr,'aerosol:fa')
        call getmem3d(ga,1,npoints,0,kz,1,numtr,'aerosol:ga')
        call getmem3d(tx,1,npoints,0,kz,1,numtr,'aerosol:tx')
        call getmem3d(uaer,1,npoints,0,kz,1,numtr,'aerosol:uaer')
        call getmem3d(wa,1,npoints,0,kz,1,numtr,'aerosol:wa')
        call getmem2d(faer,1,npoints,1,numtr,'aerosol:faer')
        call getmem2d(gaer,1,npoints,1,numtr,'aerosol:gaer')
        call getmem2d(tauaer,1,npoints,1,numtr,'aerosol:tauaer')
        call getmem2d(utaer,1,npoints,1,numtr,'aerosol:utaer')
        call getmem2d(waer,1,npoints,1,numtr,'aerosol:waer')
        if ( isorgam == 1 ) then
          call getmem2d(kasoa,1,numtr,1,nband,'aerosol:kasoa')
          call getmem2d(wasoa,1,numtr,1,nband,'aerosol:wasoa')
          call getmem2d(gasoa,1,numtr,1,nband,'aerosol:gasoa')
        end if
      end if
      ...
      if ( irrtm == 1 ) then
        ...
        if ( nbin == 4 ) then
          ...
          write(stderr,*) "SOA optical properties only"
          write(stderr,*) "available for irrtm = 0    "
          call fatal(__FILE__,__LINE__,'radiation')
        end if
        ...
        if ( isorgam == 1 ) then
          ...
          do itr = 1 , numtr
            do ns = 1 , nband
              kasoa(itr,ns) = ka_soa(itr,ns)
              wasoa(itr,ns) = wa_soa(itr,ns)
              gasoa(itr,ns) = ga_soa(itr,ns)
            end do
          end do
        end if
      end if
...
!
  ...
  subroutine aeroppt(rh,aermmr,pint,n1,n2)
    ...
    integer(ik4) :: n , l , ibin , jbin , itr , k , k1, k2 , ns , nband , numtr
      ...
      do k = 1 , kz
        do n = n1 , n2
          ...
          do itr = 1 , numtr
            ...
            else if ( isorgam == 1 ) then ! assuming all SOAs as Hydrophobic
              uaer(n,k,itr) = aermmr(n,k,itr)*path
              tx(n,k,itr) = d10e5*uaer(n,k,itr)*kasoa(itr,ns)
              wa(n,k,itr) = wasoa(itr,ns)
              ga(n,k,itr) = gasoa(itr,ns)
              fa(n,k,itr) = gasoa(itr,ns)*gasoa(itr,ns)
            end if
