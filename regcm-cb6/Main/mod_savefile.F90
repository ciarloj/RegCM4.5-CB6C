...
module mod_savefile
  ...
  integer(ik4) , parameter :: maxdims = 20
  ...
  integer(ik4) , parameter :: idtotch = 17
  integer(ik4) , parameter :: idnsoatr = 18
  integer(ik4) , parameter :: idngstr = 19
  integer(ik4) , parameter :: id3xtch = 20 !3x totch
  ...
  subroutine write_savefile(idate)
    ...
    if ( myid == iocpu ) then
      ...
      if ( ichem == 1 ) then
        ...
        if ( igaschem == 1 .and. ichsolver > 0 ) then
          ...
          if ( chemsimtype(1:4) == 'CB6C' ) then
            ncstatus = nf90_def_dim(ncid,'totch',totch,dimids(idtotch))
            call check_ok(__FILE__,__LINE__,'Cannot create dimension totch')
            if ( isorgam == 1 ) then
              ncstatus = nf90_def_dim(ncid,'3*totch',3*76,dimids(id3xtch))
              call check_ok(__FILE__,__LINE__,'Cannot create dimension 3*totch')
              ncstatus = nf90_def_dim(ncid,'nsoatr',nsoatr,dimids(idnsoatr))
              call check_ok(__FILE__,__LINE__,'Cannot create dimension nsoatr')
              ncstatus = nf90_def_dim(ncid,'ngstr',ngstr,dimids(idngstr))
              call check_ok(__FILE__,__LINE__,'Cannot create dimension ngstr')
            end if
          else
      ...
      if ( ichem == 1 ) then
        ...
        if ( igaschem == 1 .and. ichsolver > 0 ) then
          wrkdim(3) = dimids(idkh)
          if ( chemsimtype(1:4) == 'CB6C' .and. isorgam /= 1 ) then
            wrkdim(4) = dimids(idtotch)
          else if ( isorgam == 1 ) then
            wrkdim(4) = dimids(id3xtch)
          else
      ...
      rktau = ktau
      ncstatus = nf90_put_att(ncid,nf90_global,'ktau',rktau)
