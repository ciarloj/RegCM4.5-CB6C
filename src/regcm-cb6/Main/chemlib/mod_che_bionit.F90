...
!!$      real, dimension(iy,jx), intent(in) :: soilph         !soilpH
    ...
    do i  = ici1 , ici2
      if ( chemsimtype(1:4) ==  'CB6C') then
        if (ivegcov(i) == 0) cycle
        if ( ichdrdepo == 1 ) then
           chiten(j,i,kz,inmox) = chiten(j,i,kz,inmox) + &
                noxflux(i)*egrav/(dsigma(kz)*1.D3)
        elseif ( ichdrdepo == 2 ) then
           ! pass the flux to BL scheme
           chifxuw(j,i,inmox) = chifxuw(j,i,inmox) + &
                noxflux(i)
        end if
        ! diagnostic source (accumulated)
        cemtrac(j,i,inmox) = cemtrac(j,i,inmox) + &
             noxflux(i)* cfdout

        if ( ichdiag == 1 ) then
           cemisdiag(j,i,kz,inmox) = cemisdiag(j,i,kz,inmox) + &
                noxflux(i)/ ( cdzq(j,i,kz)*crhob3d(j,i,kz)) * cfdout
        end if
      else  ! CBMZ and any other simulation except than CB6C
        if (ivegcov(i) == 0) cycle
        if ( ichdrdepo == 1 ) then
