...
! This module contains routines for wet scavenging of gas and aerosols
  ...
  public :: sethet , wetdepa , wetdepsoa
...
!
  subroutine sethet(j,zmid1,phis1,tfld1,strappt,convppt, &
    ...
    qin(ici1:ici2,:,1:numtr) = qin1(ici1:ici2,:,1:numtr)
    ...
    do k = ktop + 1 , kz
      ...
      !Specifically for CB6C
      if ( intrc > 0 ) vmr_hno3(:,k) = (qin(:,k,intrc)/ps2(:)) * (amd/63.012D0)
      if ( ihpox > 0 ) vmr_h2o2(:,k) = (qin(:,k,ihpox)/ps2(:)) * (amd/34.0147D0)
    end do

    if ( ihno3 > 0 .or. intrc > 0 ) then
      xhno3(:,:)  = vmr_hno3(:,:) * xhnm(:,:)
    else
      xhno3(:,:)  = d_zero
    end if
    if ( ih2o2 > 0 .or. ihpox > 0 ) then
      xh2o2(:,:)  = vmr_h2o2(:,:) * xhnm(:,:)
    else
      xh2o2(:,:)  = d_zero
    end if
    ...
    do k = ktop + 1 , kz
      ...
      else if ( iform > 0 ) then
        het_rates(:,k,iform)  = work3(:)
      end if
      if ( iisopno3 > 0 ) then
      ...
      if ( ihyac > 0 ) then
        ...
        het_rates(:,k,ipan) = work3(:)
      end if
      if ( ihydrald > 0 ) then
        het_rates(:,k,ihydrald) = work3(:)
      else if ( ihpld > 0 ) then
        het_rates(:,k,ihpld) = work3(:)
      end if
      work3(:) = dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                 d_one/(xhen_ch3ooh(:,k)*work2(:)))),d_zero)
      if ( ich3ooh > 0 ) then
        het_rates(:,k,ich3ooh)  = work3(:)
      else if ( imepx > 0 ) then
        het_rates(:,k,imepx)  = work3(:)
      end if
      if ( ipooh > 0 ) then
      ...
      else if ( ietoh > 0 ) then
        het_rates(:,k,ietoh) = work3(:)
      end if
      if ( ic3h7ooh > 0 ) then
      ...
      else if ( irpox > 0 ) then
        het_rates(:,k,irpox) = work3(:)
      end if
      if ( ich3oh > 0 ) then
        het_rates(:,k,ich3oh) = work3(:)
      else if ( imeoh > 0 ) then
        het_rates(:,k,imeoh) = work3(:)
      end if
      if ( ieoh > 0 ) then
      ...
      else if ( ipacd > 0 ) then
        het_rates(:,k,ipacd) = dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                   d_one/(xhen_ch3co3h(:,k)*work2(:)))),d_zero)
      end if
      if ( ihno4 > 0 ) then
      ...
      if ( imvk > 0 ) then
        ...
        het_rates(:,k,iket) = dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                   d_one/(xhen_mvk(:,k)*work2(:)))),d_zero)
      end if
      if ( imacr > 0 ) then
        het_rates(:,k,imacr) = dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                   d_one/(xhen_macr(:,k)*work2(:)))),d_zero)
      else if ( iispd > 0 ) then
        het_rates(:,k,iispd) = dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                   d_one/(xhen_macr(:,k)*work2(:)))),d_zero)
      end if
      if ( ih2o2 > 0 ) then
      ...
      else if ( ihpox > 0 ) then
        work3(:) = satf_h2o2 * dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                   d_one/(xhen_h2o2(:,k)*work2(:)))),d_zero)
        het_rates(:,k,ihpox) =  work3(:) + tmp_hetrates(:,k,1)
      end if
      ...
      else if ( intrc > 0 ) then
        het_rates(:,k,intrc) = work3(:)
      end if
      ! ONIT is organic nitrate
      if ( ionit > 0 ) then
        het_rates(:,k,ionit) = work3(:)
      else if ( iintr > 0 ) then
        het_rates(:,k,iintr) = work3(:)
      end if
       !**** PB is lead
      if ( ipb > 0 ) then
      ...
      else if ( isdio > 0 ) then
        het_rates(:,k,isdio) = het_rates(:,k,ihpox)
      end if
...
!	    end if
      ...
      else if ( iaacd > 0 ) then
        het_rates(:,k,iaacd) = dmax1(rain(:,k)/(h2o_mol*(work1(:) + &
                  d_one/(xhen_ch3cooh(:,k)*work2(:)))),d_zero)
      end if
...
!
    ...
    call blcld(mbin,1,mbin,indp,rhsize,t,pressg,shj,rho,totppt,   &
               pdepv,rhop,wetdep,colef)
  ...
  subroutine wetdepsoa(j,mbin,n1bin,n2bin,beffdiam,rhoaer,t,wl,fracloud, &
                     fracum,pressg,shj,rho,strappt,convppt,pdepv,indp)
    implicit none
!
    integer(ik4) , intent(in) :: j , mbin , n1bin , n2bin
    integer(ik4) , dimension(mbin) , intent(in) :: indp
    real(rk8) , dimension(mbin) , intent(in) :: beffdiam
    real(rk8) , intent(in) :: rhoaer ! specific aerosol density
    real(rk8) , dimension(ici1:ici2,kz) , intent(in) :: wl , t , rho , &
    ...
    do n = n1bin , n2bin
      ...
      ! Important: convective, large scale, washout and rainout fluxes
      ! of every vertical level are summed for surface flux calculation.
      ! cwetdepflx(:,:,n) is also a time accumulated array which is
      ! set to 0 when the surface (CLM45) scheme is called. The average between
      ! surface time steps is calculated in the atm to surface interface
        ...
        do k = 1 , kz
          do i = ici1 , ici2
            if ( wl(i,k) > clmin ) then
              wetrem(n) = d_zero
              if ( cremrat(j,i,k) > d_zero .and. fracloud(i,k) > d_zero ) then
                wetrem(n) = fracloud(i,k)*chtrsol(n) * &
                   chib(j,i,k,n)* &
                   (dexp(-cremrat(j,i,k)/fracloud(i,k)*dt)-d_one)
                chiten(j,i,k,n) = chiten(j,i,k,n) + &
                   wetrem(n)/dt
                ...
                ! in (Kg/m2/s) passed to CLM surface scheme ( change sign
                ! convention to get a positive deposition flux)
                cwetdepflx(j,i,n)  = cwetdepflx(j,i,n) - &
                   wetrem(n)/ dt *cdzq(j,i,k)*crhob3d(j,i,k) /cpsb(j,i)
                ! save the tendency as a diag (accumulated for average at output
                ! time step)
                rainout(j,i,k,n) = rainout(j,i,k,n) + &
                   wetrem(n)/dt  *cfdout
              end if
            end if
          end do
      ...
      if ( ichremcvc == 1 ) then
        ...
        do i = ici1 , ici2
          if ( kcumtop(j,i) >  0 ) then
            do k = kcumtop(j,i) , kz
              wetrem_cvc(n) = fracum(i,k)*chtrsol(n) * &
                   chib(j,i,k,n)*(dexp(-remcum*dt)-d_one)
              chiten(j,i,k,n) = chiten(j,i,k,n) + &
                   wetrem_cvc(n)/dt
              ! add to large scale and sum up the flux on the vertical to
              ! get instantaneous surface flux in (Kg/m2/s) passed to CLM
              ! surface scheme ( change sign convention to get a positive flux)
              cwetdepflx(j,i,n)  = cwetdepflx(j,i,n) - &
                wetrem_cvc(n)/ dt *cdzq(j,i,k)*crhob3d(j,i,k) /cpsb(j,i)
              ! add the concvetive rainout to large scale save the
              ! tendency as a diag
              rainout(j,i,k,n) = rainout(j,i,k,n) + &
                   wetrem_cvc(n)/ dt *cfdout
            end do
          end if
        end do
      end if
    ...
    do i = ici1 , ici2
      do k = 1 , kz
        rhsize(i,k,n1bin:n2bin) = d_half * 1.D-06 * beffdiam(n1bin:n2bin)
      end do
    end do
    ! dry density for now
    rhop(:,:,:) = rhoaer
    ...
    ! add the contribution of strat prec
    totppt(:,:) = strappt(:,:) + totppt(:,:)
    call blcld(mbin,n1bin,n2bin,indp,rhsize,t,pressg,shj,rho,totppt,   &
               pdepv,rhop,wetdep,colef)
    ...
    do n = n1bin , n2bin
      do k = 1 , kz
        do i = ici1 , ici2
          wtend = chib(j,i,k,n)*(d_one-dexp(-wetdep(i,k,n)*dt))/dt
          chiten(j,i,k,n) = chiten(j,i,k,n) - wtend
          ...
          cwetdepflx(j,i,n)  = cwetdepflx(j,i,n) + &
               wtend * cdzq(j,i,k) * crhob3d(j,i,k) / cpsb(j,i)
          ! wet deposition washout diagnostic ( both from conv and large scale)
          washout(j,i,k,n) = washout(j,i,k,n) - wtend * cfdout
        end do
      end do
    end do
    ...
    do n = n1bin , n2bin
      do i = ici1 , ici2
        ...
        do k = 1 , kz
          ...
          ! also change sign convention
          ! normalise by psb to get the right flux unit
          wdrout(j,i,n) = wdrout(j,i,n) - &
            rainout(j,i,k,n)*cdzq(j,i,k) *crhob3d(j,i,k) /cpsb(j,i)
          wdwout(j,i,n) = wdwout(j,i,n) - &
            washout(j,i,k,n)*cdzq(j,i,k) *crhob3d(j,i,k)/cpsb(j,i)
        end do
      end do
    end do

  end subroutine wetdepsoa


  subroutine blcld(mbin,n1bin,n2bin,indp,rhsize,t,pressg,shj,rho,    & 
                   totppt,pdepv,rhop,wetdep,colef)
    implicit none

    integer(ik4) , intent(in) :: mbin , n1bin , n2bin
    ...
    call cas(t,colef,indp,rhop,rho,rhsize,pressg,mbin,n1bin,n2bin,totppt, &
             pdepv,shj)
...
!***********************************************************************
!
  subroutine cas(t,colef,indp,rhop,rho,rhsize,pressg,mbin,n1bin,n2bin,  &
                 totppt,pdepv,shj)
  !------------------------------------------------------------------------
  !  purpose:
  !  --------
