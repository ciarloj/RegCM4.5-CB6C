  ...
  subroutine init_output_streams(lparallel)
    ...
    do nstream = 1 , maxstreams
      ...
      if ( nstream == che_stream ) then
        ...
        if ( isorgam == 1 ) then
          outstream(che_stream)%nfiles = nsoatr
        else
        ...
        if ( isorgam == 1 ) then
          do itr = 3*ngstr+1, ntr
            outstream(che_stream)%cname_base(itr-3*ngstr) = chtrname(itr)
          end do
        else
          do itr = 1 , ntr
    ...
    do i = 1 , maxstreams
      ...
      do j = 1 , outstream(i)%nfiles
        ...
        if ( ichem == 1 ) then
          ...
          call outstream_addatt(outstream(i)%ncout(j), &
            ncattribute_integer('chem_activate_sorgam',isorgam))
          call outstream_addatt(outstream(i)%ncout(j), &
