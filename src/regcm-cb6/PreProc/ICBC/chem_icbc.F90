...
!
  ...
  integer(ik4) :: ichremlsc , ichremcvc , ichdrdepo , ichcumtra , &
        ...
        iindirect , ichebdy , ichjphcld , ichbion , isorgam
  real(rk8) :: rdstemfac
  logical :: dochem , doche6 , dooxcl , doaero
  data dochem /.false./
  data doche6 /.false./
  data dooxcl /.false./
  data doaero /.false./
!
  namelist /chemparam/ chemsimtype , ichremlsc , ichremcvc , ichdrdepo , &
     ...
     ichsursrc , ichebdy , rdstemfac , ichjphcld , ichbion , isorgam
...
!
  ...
  if ( doche6 ) call newfile_c6_icbc(idate)
  ...
  if ( doche6 .and. chemtyp .eq. 'MZ6HR' ) call header_ch_icbc(idate)
  if ( doche6 .and. chemtyp .eq. 'MZCLM' ) call header_ch_icbc_clim(idate)
  ...
  do nnn = 1 , nsteps
   if (.not. lsamemonth(idate, iodate) ) then
     ...
     if ( doche6 ) call newfile_c6_icbc(monfirst(idate))
   ...
   if ( doche6 .and. chemtyp .eq. 'MZ6HR' ) call get_c6_icbc(idate)
   if ( doche6 .and. chemtyp .eq. 'MZCLM' ) call get_c6_icbc_clim(idate)
  ...
  if ( doche6 .and. chemtyp .eq. 'MZ6HR' ) call close_ch_icbc
  if ( doche6 .and. chemtyp .eq. 'MZCLM' ) call close_ch_icbc_clim
