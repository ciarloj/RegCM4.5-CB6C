...
module mod_wrtoxd
  ...
  public :: newfile_ch_icbc , newfile_c6_icbc , newfile_ox_icbc , newfile_ae_icbc
  public :: write_ch_icbc , write_c6_icbc , write_ox_icbc , write_ae_icbc

  character(len=256) :: ofname
  character(len=8) :: chtype

  integer(ik4) , parameter :: nchsp = 56   !species in Mozart output
  integer(ik4) , parameter :: ncbmz = 33   !cbmz species in chemistry lateral boundaries
  integer(ik4) , parameter :: ncb6  = 54   !cb6 species in chemistry lateral boundaries
  ...
  character(len=8) , dimension(ncb6)  :: cb6spec     !Name of CB6C species
  ...
  data chspec / 'NO      ','NO2     ','N2O5    ','HNO3    ','HO2NO2  ',         &
                ...
                'O       ','O1D     ','NO3     ','H2      ','OH      ',         &
                'HO2     ','CH3O2   ','C2H5OH  ','CH3CO3  ','GLYOXAL ',         &
                'ENEO2   ','ISOPO2  ','ISOPOOH ','XOOH    ','C10H16  ',         &
                'TOLO2   ' /
 ...
 data cbmzspec / 'O3      ','NO      ','NO2     ','HNO3    ','HNO4    ','N2O5    ','H2O2    ','CH4     ',  &
  ...
  data cb6spec / 'NMOX    ','NDOX    ','DNPO    ','NTRC    ','PNA     ',    &
  ...
  type(ncvariable3d_real) , save , dimension(ncb6)  :: v3dvar_c6
  ...
  subroutine init_outoxd(chemsimtype)
    ...
    logical :: doaero , dochem , doche6 , dooxcl
    integer(ik4) :: i
    data doaero /.false./
    data dochem /.false./
    data doche6 /.false./
    ...
    if ( doche6 ) call getmem4d(chv4,1,jx,1,iy,1,kz,1,ncb6,'mod_wrtoxd:chv4')
  ...
  subroutine newfile_c6_icbc(idate1)
    implicit none
    type(rcm_time_and_date) , intent(in) :: idate1
    type(ncoutstream_params) :: opar
    integer(ik4) :: ivar
    ...
    call outstream_addvar(ncoutch,v2dvar_base(2))
    do ivar = 1 , ncb6
      v3dvar_c6(ivar)%vname = cb6spec(ivar)
      v3dvar_c6(ivar)%vunit = 'kg kg-1'
      v3dvar_c6(ivar)%long_name = trim(cb6spec(ivar))//' Volume Mixing Ratio'
      v3dvar_c6(ivar)%standard_name = &
        'mass_fraction_of_'//trim(cb6spec(ivar))//'_in_air'
      v3dvar_c6(ivar)%lrecords = .true.
      v3dvar_c6(ivar)%is_slice = .true.
      v3dvar_c6(ivar)%rval_slice => chv4
      call outstream_addvar(ncoutch,v3dvar_c6(ivar))
    end do
    call outstream_enable(ncoutch,sigmah)
    call outstream_writevar(ncoutch,v2dvar_base(1))
    call outstream_writevar(ncoutch,v2dvar_base(2))
  end subroutine newfile_c6_icbc
  ...
  subroutine write_c6_icbc(idate)
    implicit none
    type(rcm_time_and_date) , intent(in) :: idate
    integer(ik4):: ivar
    call outstream_addrec(ncoutch,idate)
    do ivar = 1 , ncb6
      call outstream_writevar(ncoutch,v3dvar_c6(ivar),is=ivar)
    end do
    write (stdout ,*) 'Write ch_icbc : ', tochar(idate)
  end subroutine write_c6_icbc
